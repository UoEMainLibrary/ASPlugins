...
class EADConverter < Converter
  ...
  Log.info('Hello after requier')
  require_relative 'lib/xml_sax'
  Log.info('Hello after requier relatice') 
  include ASpaceImport::XML::SAX
  Log.info('Hello after include') 
  ...
  Log.info('Running ead converter plugin')
  ...
  def self.configure
    ...
    (0..12).to_a.map {|i| "c" + (i+100).to_s[1..-1]}.push('c').each do |c|
      ...
      Log.info('RESOURCE ' + ancestor(:resource) + ' REF ID ' + att('id'))
    end

    Log.info('INNER XML ' + inner_xml)
    ...
    with 'unitdate' do |node|
      ...
      Log.info('Date inner xml' + inner_xml)
      make :date, {
